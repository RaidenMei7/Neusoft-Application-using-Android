...
import com.example.zyfypt_no7_406ml.adapters.Latest_VideoAdapter;
import com.example.zyfypt_no7_406ml.bean.Video;
import com.example.zyfypt_no7_406ml.iface.Latest_VideoListener;
import com.example.zyfypt_no7_406ml.model.Latest_VideoModel;
...
public class Fragment31 extends BaseFragment {
    ...
    private Latest_VideoAdapter latest_videoAdapter;
    private int page=1; // 代表页数，并初始化为1，代表第1页。
    private int lastVisibleItemPosition;//最后一条可见条目的位置



    private Latest_VideoListener latest_videoListener = new Latest_VideoListener() {
        @Override
        public void onResponse(List<Video> videolist) {
            ...
            latest_videoAdapter = new Latest_VideoAdapter(list,context);
            recyclerView.setAdapter(latest_videoAdapter);
        }
        @Override
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        System.out.println("--f31--"+getSessionId());//getSessionId是父类的方法

        recyclerView = view.findViewById(R.id.fragment31);
    ...
    public void getData() {
        Latest_VideoModel latest_videoModel=new Latest_VideoModel();
        latest_videoModel.getVideoList("video",page,getSessionId(),latest_videoListener);
    }
}