...
public class CollectVideoAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    private List<CollectResult<Video>> list;//数据源
    private Context context;//上下文
    public CollectVideoAdapter(Context context,List<CollectResult<Video>> list) {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        Video video = list.get(position).getBean();
        if (video == null)  return;
        CollectVideoAdapter.ViewHolder viewHolder = ( CollectVideoAdapter.ViewHolder) holder;
        ...
        viewHolder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                //取出当前item的id
                int id=video.getId();//bean需要增加final
