...
public class CollectArticleAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    private List<CollectResult<Article>> list;//数据源
    private Context context;//上下文
    public CollectArticleAdapter(Context context,List<CollectResult<Article>> list) {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, @SuppressLint("RecyclerView") int position) {
        Article article = list.get(position).getBean();
        if (article == null)  return;
        CollectArticleAdapter.ViewHolder viewHolder = ( CollectArticleAdapter.ViewHolder) holder;
        ...
        viewHolder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                //取出当前item的id
                int id=article.getId();//bean需要增加final
